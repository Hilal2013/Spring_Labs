...
public class CartServiceImpl implements CartService {
    ...
    public CartServiceImpl(CartRepository cartRepository, ProductRepository productRepository, MapperUtil mapperUtil, CartItemRepository cartItemRepository, DiscountRepository discountRepository) {
...
//cart->already has 5milk and you will add more  // trying to find any product in cart?
        // if product is already added to cart before, we will update the quantity, adding to existing quantity value
        if (cartItem != null) {
            //getquqntity->existing quantity and quantity->new
    ...
    public BigDecimal applyDiscountToCartIfApplicableAndCalculateDiscountAmount(String discountName, Cart cart) {
        ...
        //retrieving all cart items belongs to specific cart
    ...
    public BigDecimal calculateTotalCartAmount(List<CartItem> cartItemList) {
       ...
       //multiply price and cartitem (not product) quantity
...
//and sum
    ...
    private Cart checkCartCount(List<Cart> cartList, Customer customer) {
        //is there any cards related with this customer
        // if customer doesn't have cart before adding a product to cart,
        // no need to throw exception. We can create one.
        ...
        return cartList.get(0);// first index of cartlist because we have only one cart
    }
}
